...
interface Props {
    ...
    width?: number;
    justifyContet?: string;
}

export const TagPickerInline = ({ allowNewTags, onSelectedTagsChange, preSelectedTags, width, justifyContet }: Props) => {
    ...
    const filterSuggestedTags = (filterText: string, tagList: ITag[] | undefined): ITag[] => {
        var existingMatches = filterText
            ? tags.filter(tag => tag.name.toLowerCase().indexOf(filterText.toLowerCase()) === 0 && !listContainsTagList(tag, tagList))
            : [];
    ...
    return (
        <div className={styles.tagArea}>
            <Typography component={"div"} sx={{ justifyContent: justifyContet ?? "center" }} className={styles.tagSelection}>
                <div className={allowAddNew ? styles.rootClass : styles.rootClassFilter}>
                    <label className={styles.tagName} htmlFor={pickerId}>
                        {t("customize.panel.tags")}
                    </label>
                    <TagPicker
                ...
                <TooltipHost
                    content={allowAddNew ? t("customize.panel.tagUpload") : t("customize.panel.tagFilter")}
                    styles={hostStyles}
                    id={tooltipId}
                >
