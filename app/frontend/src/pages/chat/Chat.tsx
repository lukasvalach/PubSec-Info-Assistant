...
import { Checkbox, Panel, Separator, SpinButton, TextField } from "@fluentui/react";
import { LockClosedFilled, PersonStarFilled, SparkleFilled } from "@fluentui/react-icons";
...
const Chat = () => {
    ...
    const [userPersona, setUserPersona] = useState<string>(t("analyst"));
    const [systemPersona, setSystemPersona] = useState<string>(t("assistant"));
        ...
        <Box display="flex" minHeight="100vh">
            ...
            <div className={styles.container}>
                {!!lastQuestionRef.current && (
                    <Box display="flex">
                        ...
                        <Box display={"block"}>
                            ...
                            <Typography component="p" fontSize={10} fontWeight="small">
                ...
                <div className={styles.chatRoot}>
                    <div className={styles.chatContainer}>
                        {!lastQuestionRef.current ? (
                            <div className={styles.chatEmptyState}>
                                <img src={phone} alt="chat" className={styles.headerLogo} />
                                <h1 className={styles.chatEmptyStateTitle}>{t("chat.title")}</h1>
                                <span className={styles.chatEmptyObjectives}>{t("chat.intro")}</span>
                                <Box className={styles.chatEmptyObjectivesList}>
                                    <span className={styles.chatEmptyObjectivesListItem}>
                                        <SparkleFilled fontSize={"40px"} primaryFill={"rgba(115, 118, 225, 1)"} aria-hidden="true" aria-label="SparkleFilled" />
                                        <Typography variant="h6" sx={{ fontWeight: "bold", m: 1 }}>
                                            {t("chat.emptyState.simple.title")}
                                        </Typography>
                                        <Typography sx={{ fontSize: 14, fontStyle: "italic" }} className={styles.chatEmptyObjectivesListItemText}>
                                            {t("chat.emptyState.simple.text")}
                                        </Typography>
                                    </span>
                                    <span className={styles.chatEmptyObjectivesListItem}>
                                        <PersonStarFilled
                                        ...
                                        />
                                        <Typography variant="h6" sx={{ fontWeight: "bold", m: 1 }}>
                                            {t("chat.emptyState.relevant.title")}
                                        </Typography>
                                        <Typography sx={{ fontSize: 14, fontStyle: "italic" }} className={styles.chatEmptyObjectivesListItemText}>
                                            {t("chat.emptyState.relevant.text")}
                                        </Typography>
                                    </span>
                                    <span className={styles.chatEmptyObjectivesListItem}>
                                        <LockClosedFilled
                                        ...
                                        />
                                        <Typography variant="h6" sx={{ fontWeight: "bold", m: 1 }}>
                                            {t("chat.emptyState.safe.title")}
                                        </Typography>
                                        <Typography sx={{ fontSize: 14, fontStyle: "italic" }} className={styles.chatEmptyObjectivesListItemText}>
                                            {t("chat.emptyState.safe.text")}
                                        </Typography>
                                    </span>
                                </Box>
                            ...
                            <div className={styles.chatMessageStream}>
                                {answers.map((answer, index) => (
                                    <div key={index}>
                                        ...
                                        <Box>
                                            {/* TODO: refactor this */}
                                            <Typography component={"div"} sx={{ marginLeft: 7, fontWeight: 600, fontSize: 14 }}>
                                                Databot
                                            </Typography>
                                            <Box sx={{ display: "flex" }}>
                        ...
                        <Typography component={"div"} sx={{ maxWidth: "1000px" }} className={styles.chatInput}>
                            <QuestionInput
                                clearOnSend
                    ...
                    <Panel
                        headerText={t("customize.panel.title")}
                    ...
                    >
                        <Separator className={styles.chatSettingsSeparator}></Separator>
                        <FolderPicker allowFolderCreation={false} width={290} onSelectedKeyChange={onSelectedKeyChanged} preSelectedKeys={selectedFolders} />
                        <SpinButton
                            className={styles.chatSettingsSeparator}
                            label={t("customize.panel.documentRetrieve")}
                            min={1}
                            max={50}
                            styles={{ label: { paddingBottom: "3rem" } }}
                        ...
                        <Checkbox
                            ...
                            label={t("customize.panel.suggestQuestions")}
                        ...
                        <TextField
                            ...
                            label={t("customize.panel.userPersona")}
                            onChange={onUserPersonaChange}
                        />
                        <TextField
                            className={styles.chatSettingsSeparator}
                            defaultValue={systemPersona}
                            label={t("customize.panel.systemPersona")}
                            onChange={onSystemPersonaChange}
                        />
                    </Panel>

                    <Panel
                        headerText={t("customize.panel.information")}
